...
mixin SkipTimelineTileNode on Widget {
  /// {@template timelines.node.position}
  /// If this is null, then the [SkipTimelineThemeData.nodePosition] is used.
  /// {@endtemplate}
  double? get position;
  double getEffectivePosition(BuildContext context) {
    return position ?? SkipTimelineTheme.of(context).nodePosition;
  }
}
...
class SkipTimelineNode extends StatelessWidget with SkipTimelineTileNode {
  /// Creates a timeline node.
  ///
  /// The [indicatorPosition] must be null or a value between 0 and 1.
  const SkipTimelineNode({
    Key? key,
    this.direction,
    this.startConnector,
    this.endConnector,
    this.indicator = const SkipContainerIndicator(),
  ...
  double _getEffectiveIndicatorPosition(BuildContext context) {
    ...
    indicatorPosition ??= (indicator is SkipPositionedIndicator)
        ? (indicator as SkipPositionedIndicator).getEffectivePosition(context)
        : SkipTimelineTheme.of(context).indicatorPosition;
    return indicatorPosition;
  }

  bool _getEffectiveOverlap(BuildContext context) {
    var overlap = this.overlap ?? SkipTimelineTheme.of(context).nodeItemOverlap;
    return overlap;
  }

  @override
  Widget build(BuildContext context) {
    final direction = this.direction ?? SkipTimelineTheme.of(context).direction;
    ...
    final lineItems = [
      if (indicatorFlex > 0)
        Flexible(
          ...
          child: startConnector ?? SkipTransparentConnector(),
        ),
      if (!overlap) indicator,
      if (indicatorFlex < 1)
        Flexible(
          flex: ((1 - indicatorFlex) * kFlexMultiplier).toInt(),
          child: endConnector ?? SkipTransparentConnector(),
        ),
    ];
    ...
    if (overlap) {
      ...
      final positionedIndicatorItems = [
        if (indicatorFlex > 0)
          Flexible(
            flex: (indicatorFlex * kFlexMultiplier).toInt(),
            child: SkipTransparentConnector(),
          ),
        indicator,
        Flexible(
          flex: ((1 - indicatorFlex) * kFlexMultiplier).toInt(),
    ...
    if (SkipTimelineTheme.of(context).direction != direction) {
      result = SkipTimelineTheme(
        data: SkipTimelineTheme.of(context).copyWith(
          direction: direction,
        ),
        child: result,
      );
